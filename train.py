...
def defaultdict_from_json(jsonDict):
    def func(): return defaultdict(str)
    dd = func()
    dd.update(jsonDict)
    return dd
...
def train(training_dataset_loader, testing_dataset_loader, args, data_len, sub_class, class_type, device):
    ...
    seg_model = SegmentationSubNetwork(
        in_channels=6, out_channels=1).to(device)

    optimizer_ddpm = optim.Adam(unet_model.parameters(
    ), lr=args['diffusion_lr'], weight_decay=args['weight_decay'])

    optimizer_seg = optim.Adam(seg_model.parameters(
    ), lr=args['seg_lr'], weight_decay=args['weight_decay'])
    ...
    for epoch in tqdm_epoch:
        ...
        for i, sample in enumerate(tbar):
            ...
            noise_loss, pred_x0, normal_t, x_normal_t, x_noiser_t = ddpm_sample.norm_guided_one_step_denoising(               model=unet_model, x_0=aug_image, anomaly_label=anomaly_label, anomaly_mask=anomaly_mask, args=args)
            pred_mask = seg_model(torch.cat((aug_image, pred_x0), dim=1))

            # print("normal_t:", normal_t)
            # print("x_noiser_t:", x_noiser_t)
            ...
            tbar.set_description('Epoch:%d, Train loss: %.3f' %
                                 (epoch, train_loss))
        ...
        if (epoch+1) % 50 == 0 and epoch > 0:
            temp_image_auroc, temp_pixel_auroc = eval(
                testing_dataset_loader, args, unet_model, seg_model, data_len, sub_class, device)
            ...
            if (temp_image_auroc+temp_pixel_auroc >= best_image_auroc+best_pixel_auroc):
                if temp_image_auroc >= best_image_auroc:
                    save(unet_model, seg_model, args=args,
                         final='best', epoch=epoch, sub_class=sub_class)
                    best_image_auroc = temp_image_auroc
                    best_pixel_auroc = temp_pixel_auroc
                    best_epoch = epoch

    save(unet_model, seg_model, args=args, final='last',
         epoch=args['EPOCHS'], sub_class=sub_class)

    temp = {"classname": [sub_class], "Image-AUROC": [best_image_auroc],
            "Pixel-AUROC": [best_pixel_auroc], "epoch": best_epoch}
...
def eval(testing_dataset_loader, args, unet_model, seg_model, data_len, sub_class, device):
    ...
    seg_model.eval()
    os.makedirs(
        f'{args["output_path"]}/metrics/ARGS={args["arg_num"]}/{sub_class}/', exist_ok=True)
    in_channels = args["channels"]
    betas = get_beta_schedule(args['T'], args['beta_schedule'])
    ...
    for i, sample in enumerate(tbar):
        ...
        normal_t_tensor = torch.tensor(
            [args["eval_normal_t"]], device=image.device).repeat(image.shape[0])
        noiser_t_tensor = torch.tensor(
            [args["eval_noisier_t"]], device=image.device).repeat(image.shape[0])
        ...
        total_image_pred = np.append(
            total_image_pred, image_score.detach().cpu().numpy())
        total_image_gt = np.append(
            total_image_gt, target[0].detach().cpu().numpy())

        flatten_pred_mask = out_mask[0].flatten().detach().cpu().numpy()
        flatten_gt_mask = gt_mask[0].flatten(
...
def main():
    ...
    mpdd_classes = ['bracket_black', 'bracket_brown',
    ...
    for sub_class in current_classes:
        ...
        training_dataset_loader = DataLoader(
            training_dataset, batch_size=args['Batch_Size'], shuffle=True, num_workers=1, pin_memory=True, drop_last=True)
        ...
        train(training_dataset_loader, test_loader, args,
              data_len, sub_class, class_type, device)
