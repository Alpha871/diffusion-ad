...
class MVTecTrainDataset(Dataset):
    ...
    def __init__(self, data_path, classname, img_size, args):
        ...
        self.anomaly_source_paths = sorted(
            glob.glob(self.anomaly_source_path+"/images/*/*.jpg"))

        # Remplacer imgaug par albumentations
        self.augmenters = [
        ...
        self.augmenters_mask = [
            ...
            A.Affine(shear=(-40, 40), p=1.0),
            A.ShiftScaleRotate(shift_limit=0.5, scale_limit=0,
        ...
        self.textural_foreground_path = sorted(
            glob.glob(foreground_path + "/thresh/*.png"))

    def __len__(self):
        return len(self.image_paths)

    def random_choice_foreground_path(self):
        foreground_path_id = torch.randint(
            0, len(self.textural_foreground_path), (1,)).item()
    ...
    def randAugmenter_anomaly(self):
        ...
        return A.Compose([
            self.augmenters_anomaly[aug_ind[0]],
    ...
    def randAugmenter_mask(self):
        ...
        return A.Compose([self.augmenters_mask[aug_ind[0]]])
    ...
    def perlin_synthetic(self, image, thresh, anomaly_source_path, cv2_image, thresh_path):
        ...
        if no_anomaly > 0.5:
            ...
            while (has_anomaly == 0 and try_cnt < 50):
                ...
                object_perlin = np.expand_dims(
                    object_perlin, axis=2).astype(np.float32)
            ...
            if self.classname in texture_list:  # only DTD
                ...
                anomaly_source_img = cv2.cvtColor(
                    cv2.imread(anomaly_source_path), cv2.COLOR_BGR2RGB)
                ...
                if texture_or_patch > 0.5:  # >0.5 is DTD
                    ...
                    anomaly_source_img = cv2.resize(anomaly_source_img, dsize=(
                        self.resize_shape[1], self.resize_shape[0]))
                    # Utiliser albumentations
                    anomaly_img_augmented = aug(
                        image=anomaly_source_img)['image']
                    img_object_thr = anomaly_img_augmented.astype(
                        np.float32) * object_perlin/255.0
